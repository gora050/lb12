...
class Polynomial :
    ...
    def __mul__(self, rhsPoly):
        ...
        while p1 is not None:
            p2 = rhsPoly._polyHead
            while p2 is not None:
                if p1.degree + p2.degree not in deg:
                    deg[p2.degree+p1.degree] = p1.coefficient * p2.coefficient
                else:
                    deg[p1.degree+p2.degree] += p1.coefficient * p2.coefficient
                    p2 = p2.next
                p1 = p1.next
        newPoly = Polynomial()
        for elem in reversed(sorted(deg.keys())):
            newPoly._appendTerm(elem, deg[elem])
        return newPoly
    ...
    def __str__(self):
        curr = self._polyHead
        res = str(curr.coefficient) + "x^" + str(curr.degree)
        curr = curr.next
